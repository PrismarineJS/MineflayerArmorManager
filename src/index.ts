...
const initializeBot: Plugin = (bot, options) => {
  ...
  bot.on('playerCollect', function onPlayerCollect(collector, item) {
    ...
    try {
      ...
      var itemId = 'itemId' in itemMetadata ? itemMetadata.itemId : 'blockId' in itemMetadata && itemMetadata.blockId;
      if(itemId && collector.username === bot.username && isArmor(itemId, versionData)) {
